...
class AlfalfaVariables < OpenStudio::Measure::ModelMeasure
  ...
  def run(model, runner, user_arguments)
    ...
    create_input(model, "FanOnOff")
    create_input(model, "CompressorOnOff")

    # Alfalfa outputs
    zone_temp = OpenStudio::Model::OutputVariable.new("Zone Mean Air Temperature", model)
    zone_temp.setName("Zone Temperature")
    zone_temp.setReportingFrequency("Timestep")
    zone_temp.setKeyValue("Zn1")
    zone_temp.setExportToBCVTB(true)
